...
export function runHiddenTests(workDir: string, task: AbTask, timeoutMs = 15000): TestScore {
  ...
  try {
    ...
    if (r.error || r.status !== 0) return { pass: 0, total, error: r.error ? String(r.error.message) : `exit ${r.status}` };
    const out = JSON.parse(r.stdout || '{}');
    return { pass: Number(out.pass) || 0, total: Number(out.total) || total, ...(out.loadError ? { loadError: out.loadError } : {}) };
  } finally {
