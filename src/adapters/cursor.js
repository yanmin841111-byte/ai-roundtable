...
function describeCursorTool(key, args = {}) {
...
function viaShell(bin, args) {
  return ['/bin/sh', ['-c', 'set -o pipefail; "$0" "$@" | cat', bin, ...args]];
}

function createCursorAdapter({ bin = 'cursor-agent' } = {}) {
  let fetched = { models: null, at: 0, error: null, pending: null };
    ...
    const res = await runProcess(...viaShell(bin, args), { cwd: ctx.cwd, stdin: prompt, timeoutMs: ctx.timeoutMs }, {
      ...
      onLine: (line) => {
        ...
        if (ev.type === 'assistant' && ev.message && Array.isArray(ev.message.content)) {
          const piece = ev.message.content.map((c) => (c && c.type === 'text' ? c.text || '' : '')).join('');
        ...
        if (ev.type === 'tool_call' && ev.tool_call) {
          ...
          const key = Object.keys(ev.tool_call).find((k) => /ToolCall$/.test(k));
          const call = (key && ev.tool_call[key]) || {};
          const activity = { id: ev.call_id || ev.tool_call.toolCallId, kind: 'tool', title: describeCursorTool(key, call.args || {}) };
