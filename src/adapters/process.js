...
function runProcess(bin, args, { cwd, stdin, shell, env, timeoutMs = DEFAULT_TURN_TIMEOUT_MS, killGraceMs = DEFAULT_KILL_GRACE_MS } = {}, cb = {}) {
...
function checkCli(bin, args = ['--version']) {
  if (!args) {
...
function runQuick(bin, args) {
  return new Promise((resolve) => {
    ...
    try { child = attachProcessGroupKill(spawn(bin, args, { detached: true, env: process.env })); } catch (e) { return finish({ ok: false, error: String(e) }); }
    child.stdout.on('data', (d) => (out += d));
    child.stderr.on('data', (d) => (out += d));
    child.on('error', (e) => finish({ ok: false, error: e.code === 'ENOENT' ? `找不到指令 ${bin}` : e.message }));
    child.on('close', (code) => finish({ ok: code === 0, out }));
  });
}

// 給非行程型轉接器(例如 HTTP API)用的停止把手,介面與 ChildProcess 一致:kill() 與 'close' 事件。
function createStopHandle(onKill) {
