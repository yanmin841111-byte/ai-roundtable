...
> A desktop app that seats multiple AI coding CLIs and APIs (Claude Code, Codex CLI, Grok, Kimi, DeepSeek, Gemini, Ollama, or anything you plug in) at one table: they debate a task, split the work, execute in parallel, and review each other's output. You watch the whole conversation live and can jump in at any time.

![AI Roundtable 畫面](docs/screenshot.png)

## 特色

- **多個 AI 同桌**:內建 Claude Code、Codex CLI;Grok、Kimi、DeepSeek、Gemini、OpenRouter、Ollama 等可從範本一鍵加入。
...
- Node.js 20 以上
- 至少一個 AI 來源:
  - [Claude Code](https://docs.anthropic.com/en/docs/claude-code) 或 [Codex CLI](https://github.com/openai/codex),安裝並登入即可直接使用
  - 或其他 CLI / API,透過擴充接入
...
| 欄位 | 說明 |
| --- | --- |
| AI CLI | 內建的 Claude Code、Codex CLI、自訂指令,或你加入的擴充 |
