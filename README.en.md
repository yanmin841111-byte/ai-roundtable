...
The output is `release/AI Roundtable-<version>.dmg`. You can also download it from [Releases](https://github.com/yanmin841111-byte/ai-roundtable/releases). There is no Apple developer certificate, so the app is unsigned: the first time, right-click the app in Finder → Open, or allow it under System Settings → Privacy & Security.
