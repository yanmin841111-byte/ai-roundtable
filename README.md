...
- **快速組隊**:從側欄「陣容」選程式開發組或文件研究組,預覽並調整規劃、執行、審查三個角色後套用;成員不足時可用 GitHub Copilot 一鍵建立三個低成本模型的唯讀成員。
- **多 AI 把關**:討論與含驗收條件的計畫都要全員同意才動工,依序執行後,每份成果由兩位以上獨立審查者全數通過才算完成;兩位成員即可使用。
...
- **多 AI 把關(至少兩位)**:討論須全員同意;主持人的計畫必須列出驗收條件,並獲其他成員全數同意才執行。執行依計畫順序一棒接一棒,後一棒收到前面的交接。每份成果由所有非作者成員以乾淨 context 逐條對照驗收條件審查;只有兩位成員時,作者另開全新上下文擔任第二位審查者。未全數通過就由原作者修正,最多三輪,每輪重新驗證並整批重審。全員同意不代表已驗收或可以上線。
- **討論 → 寫測試 → 實作 →
...
有改檔權限的成員只在執行與修復回合可以寫入;討論、分工、計畫審核、審查與總結一律唯讀。「允許成員建立 git 提交」預設關閉,改動留在工作目錄由你決定。
...
GitHub Copilot CLI 的模型清單讀自 `copilot help config`(不耗額度),`gpt-5-mini`、`claude-haiku-4.5`、`gpt-5.4-mini` 標示為低成本並排在前面;實際可用的模型由組織政策決定。「允許成員建立 git 提交」關閉時,Copilot 成員額外帶上 `--deny-tool=shell(git commit)` 與 `--deny-tool=shell(git push)`;其他 CLI 只在提示詞中要求,無法強制。
